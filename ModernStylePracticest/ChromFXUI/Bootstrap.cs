...
using ChromFXUI.ResourceHandler;
...
namespace ChromFXUI
{
	...
	public static class Bootstrap
	{
		...
		private static string appDataDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"LiuJiaHengPersonal\ChromFXUI\", Application.ProductName);
		...
		public static bool Load(Action<CfxSettings> settingsBuildAction = null, Action<CfxCommandLine> commandLineBuildAction = null)
		{
			...
			BrowserCore.OnBeforeCfxInitialize += (args) =>
			{
                ...
                //This should be enabled on next ChromFXUI version. On 2987, it will cause some issues.
			...
			BrowserCore.OnBeforeCommandLineProcessing += (args) =>
			{
                ...
                //This should be enabled on next ChromFXUI version.
